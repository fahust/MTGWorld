...
function arrayUnique(array) {
 ...
 class MyMulti extends Component {
    ...
    Alert.alert(
      ...
      [
        { text: "Add", onPress: () => {
            ...
            this.props.pro.wishlists[index] = this.props.use[index]
            storeData(this.props.pro);
            this.props.setPro({});
          } 
        },
    ...
    Alert.alert(
      ...
      [
        { text: "Add", onPress: () => {
            ...
            this.props.pro.decks[index] = this.props.use[index]
            storeData(this.props.pro);
            this.props.setPro({});
          } 
        },
  ...
  vote = (index) => {//console.log('idvote',this.props.use[index].idClient)
  ...
  dataBigCard= () =>{
    Object.keys(this.props.use).forEach((deck, itemI)=> {
      var uncomon = 0;
      var common = 0;
      var rare = 0;
      var mythic = 0;
      var price = 0;
      var colors = []
      Object.keys(this.props.use[deck].cards).forEach((card, itemI)=> {
        //https://c2.scryfall.com/file/scryfall-symbols/sets/default.svg?1627272000
        if(this.props.use[deck].cards[card].rarity == "uncommon") uncomon++
        if(this.props.use[deck].cards[card].rarity == "common") common++
        if(this.props.use[deck].cards[card].rarity == "rare") rare++
        if(this.props.use[deck].cards[card].rarity == "mythic") mythic++
        if(this.props.use[deck].cards[card].prices.eur != null) price += parseInt(this.props.use[deck].cards[card].prices.eur);
        colors = arrayUnique(colors.concat(this.props.use[deck].cards[card].color_identity));
      })
      this.props.use[deck].uncomon = uncomon
      this.props.use[deck].common = common
      this.props.use[deck].rare = rare
      this.props.use[deck].mythic = mythic
      this.props.use[deck].price = price
      this.props.use[deck].colors = colors
    })
  }
  ...
  navigateToDeck = (props,item) => {
    props.navigation.navigate(props.oneNavigate, {item: props.player?props.use[item]:item,route:props.route,cantUpdate:props.cantUpdate,player:props.player?props.player:undefined})
  }
    ...
    >
      ...
      <Center flex={1}>
        <VStack space={4} flex={1} w="100%" mt={4}>
          ...
          <VStack>
        {!this.props.cantUpdate&&Object.keys(props.use).length<=0?
          ...
          <View 
            style={{
              ...
              flex: 1}}>
            {
            (keysCard?keysCard:Object.keys(props.use)).map(function(item, itemI) {
              var cardsTemp = {}
              var BreakException = {};
              try {
                Object.keys(tthis.props.use[item].cards).forEach((card,key) => {
                  if(tthis.props.use[item].cards[card]){
                    cardsTemp[tthis.props.use[item].cards[card].name] = tthis.props.use[item].cards[card]
          ...
          {props.cantUpdate!=true?<Select
            ...
            style={{backgroundColor:"rgba(0,0,0,0.5)",width:50,fontSize:10,color:"white"}}
              selectedValue={props.use[item].mode}
              minWidth={50}
              minheight={10}
              height={10}
              ...
              mt={1}
            >
              <View>{tthis.linkFormat(props.use[item].mode)}</View>
            ...
            </Select>:<Text style={{right:0,position:"absolute",backgroundColor:"rgba(0,0,0,0.5)",fontSize:20,color:"white",fontWeight:"bold",padding:5,}}>{props.use[item].mode?props.use[item].mode.charAt(0).toUpperCase() + props.use[item].mode.slice(1):''}</Text>}
            </View>
          
          {props.cantUpdate?<Text style={{fontSize:25,fontWeight:"bold",color:"white",top:0,marginLeft:5}}>{props.use[item].title}</Text>:<Input
            shadow={9}
            color="gold"
            textColor="gold"
            ...
            defaultValue={props.use[item].title}
            variant="Click here"
          ...
          <Ripple cancelTolerance={100} color='rgb(255, 255, 255)' onPress={()=>{tthis.navigateToDeck(props,item)}}>
            <Stack space={0} p={[1, 1, 1]}>
              <Heading size={["md", "lg", "md"]} noOfLines={2}>
                {props.use[item].colors.map((color, keyColor) =>{return <View key={color+keyColor} style={{position:"absolute",right:0,paddingLeft:5,paddingRight:5}}>
            ...
            <View style={{bottom:10,left:0,right:0,flexDirection:"row",flexGrow:1,justifyContent:"center",alignContent:"center"}}>
              ...
              />
              <Text style={{marginTop:2,fontWeight:"bold",fontSize:16,color:"white"}}>{props.use[item].common}</Text>
              <SvgUri
                style={styles.innerCircle} fill={"#83AFC2"} stroke={"#83AFC2"}
                strokeWidth={2} strokeOpacity={1} strokeLinecap="butt" color={"white"} fillOpacity={1} width="18" height="20"
                uri="https://c2.scryfall.com/file/scryfall-symbols/sets/default.svg?1627272000"
              />
              <Text style={{marginTop:2,fontWeight:"bold",fontSize:16,color:"white"}}>{props.use[item].uncomon}</Text>
              <SvgUri
                style={styles.innerCircle} fill={"#CFA20B"} stroke={"#CFA20B"}
                strokeWidth={2} strokeOpacity={1} strokeLinecap="butt" color={"white"} fillOpacity={1} width="18" height="20"
                uri="https://c2.scryfall.com/file/scryfall-symbols/sets/default.svg?1627272000"
              />
              <Text style={{marginTop:2,fontWeight:"bold",fontSize:16,color:"white"}}>{props.use[item].rare}</Text>
              <SvgUri
                style={styles.innerCircle} fill={"#D15410"} stroke={"#D15410"}
                strokeWidth={2} strokeOpacity={1} strokeLinecap="butt" color={"white"} fillOpacity={1} width="18" height="20"
                uri="https://c2.scryfall.com/file/scryfall-symbols/sets/default.svg?1627272000"
              />
              <Text style={{marginTop:2,fontWeight:"bold",fontSize:16,color:"white"}}>{props.use[item].mythic}</Text>
            </View>
          </Ripple>
          
          
          {props.use[item].price!=0?<Text style={{marginTop:2,fontWeight:"bold",fontSize:18,position:"absolute",left:5,bottom:5,color:"white"}}>{props.use[item].price} eur</Text>:<></>}
          ...
          /></View>}
            {!props.cantUpdate?<></>:<View style={{flexDirection:"row",position:"absolute",bottom:0,flexGrow:1}}>
              <View style={{paddingRight:Dimensions.get('window').width/4,left:0}}>
                ...
                >
                  <Menu.Item onPress={() => {tthis.duplicate(item)}} style={{flexDirection:"row"}}><Icon name="copy" size={20} color="black" />  Duplicate to my decks</Menu.Item>
                  <Menu.Item onPress={() => {tthis.duplicateWishlist(item)}} style={{flexDirection:"row"}}><Icon name="copy" size={20} color="black" />  Duplicate to my whishlists</Menu.Item>
                </Menu>
              </View>

              <Text style={{fontWeight:"bold",marginTop:8,color:"white"}}>{props.use[item].vote}</Text>
              <IconButton
              style={{}}
              colorScheme="emerald"
